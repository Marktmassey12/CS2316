...
############################################################################
# ASNWER THE QUESTIONS BELOW. USE THE CURSOR TO EXECUTE YOUR QUERIES
############################################################################
...
def question2a(cursor, areaname, pheight, verticle, atype): # 1 point
    ...
    # cursor.execute(query)
    # return(cursor.fetchall())
    query = "INSERT INTO areas VALUES('Lionshead',12250,NULL,1900,'MISC')"
...
def question2b(cursor, data):
    ...
    '''
    data = [('Rasputins Revenge', 'Siberia', 21, 'Black', 'Ungroomed', 'Open', 'Meadow', None),('Chopstix', 'China', 36, 'Blue', 'Groomed', 'Closed', 'Run', 'Ravine')]
    return cursor.executemany("INSERT INTO runs VALUES(%s,%s,%s,%s,%s,%s,%s,%s)",data)
    

def question3a(cursor):
    '''
    Create a two new tables in vail database called area_data and restaurants
    ...
    # '''
    query1 = "DROP TABLE IF EXISTS area_data;"
    query2 = "CREATE TABLE area_data (\
    ...
    query3 = "DROP TABLE IF EXISTS restaurants;"
    query4 = "CREATE TABLE restaurants (\
              restaurant_name VARCHAR(50) NOT NULL PRIMARY KEY UNIQUE , \
              cuisine VARCHAR(20) , \
              price VARCHAR(4) , \
              type VARCHAR(50));"
...
def question3b(cursor):
    ...
    '''
    with open("vail_restaurants.csv") as f:
        reader = csv.reader(f)
        data = list(reader)
        cursor.executemany("INSERT INTO restaurants values (%s, %s, %s, %s);", data[1:])
...
def question3c(cursor):
    ...
    # ('Blue Sky', 'Black', 4)
    '''
    query = 'INSERT INTO area_data SELECT location, level, count(*) FROM runs GROUP BY location, level;'
    # on area_name =  area.name GROUP BY area.name;'
    cursor.execute(query)
...
def question4a(cursor):
    ...
    '''
    cursor.execute("UPDATE areas set bheight = pheight - verticle;")
...
def question4b(cursor):
    ...
    '''
    # cursor.execute("UPDATE lifts set num_runs = (SELECT COUNT(*) from runs where num_runs.liftnumber = runs.num_runs);")
    # cursor.execute("UPDATE media set num_characters = (SELECT COUNT(*) from characters where media.id = characters.media_id);")
    pass
