...
def query1(cursor):
    ...
    query = 'SELECT title FROM episodes WHERE (air_date > 1996 AND episode_num < 15)'
...
def query2(cursor, character_id_min):
    ...
    ((4,), (6,), (7,), (16,), (27,))
    '''
    # characterParamter = 'Update quotes set character_id_min = %s'
    # query = 'SELECT quote_id FROM quotes WHERE quote_text LIKE 'I%'
    # query = 'SELECT quote_id FROM quotes where quote_text LIKE '%I%''
    query = 'SELECT quote_id FROM quotes where character_id > %s and quote_text LIKE \'I%%\''
...
def query3(cursor):
    ...
    # query = 'SELECT char_id, char_name, age FROM characters order by age and char_id desc and max(char_id) limit 1'
    query = 'SELECT char_id, char_name, age FROM characters order by age and char_id asc limit 1 offset 8'
...
def query4(cursor):
    ...
    # query = 'SELECT building_name FROM apartments WHERE street_name LIKE '%5%''
    query = 'SELECT building_name,count(*) FROM apartments GROUP BY building_name HAVING count(*) <2'
...
def query5(cursor):
    ...
    # query = 'SELECT * from relationships WHERE char_id in (SELECT type from relationships where type LIKE '%hopeless')'
    # query = 'SELECT char1_id, char2_id from relationships WHERE type in (SELECT type from relationships where type == 'hopeless''
    # query = 'SELECT char1_id, char2_id from relationships WHERE type in (SELECT type from relationships where type = "hopeless")'
    query = 'SELECT char_name from characters WHERE char_id in (SELECT char1_id from relationships where type = "hopeless") or char_id in (SELECT char2_id from relationships where type = "hopeless")'
...
def query6(cursor):
    ...
    query = 'SELECT air_date, round((count(episodes)))'
...
def query7(cursor):
    ...
    query = 'SELECT actor_id, char_name as "char", round((count(episode_id)/25)*100,0) as ep_percentage from episodes join characters on episodes.actor_id = characters.char_id group by actor_id order by ep_percentage desc limit 3 offset 1'
